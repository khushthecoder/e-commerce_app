...
for (let i = 0; i < total; i++) {
  ...
  const image = `https://source.unsplash.com/random/800x800/?${category},${brand},product`;
  const images = [];
  const imageKeywords = [category, brand, 'product', 'studio'];
  const imgCount = randomInt(3, 5);
  for (let j = 0; j < imgCount; j++) {
    images.push(`https://source.unsplash.com/random/800x800/?${category},${brand},product,studio,${j}`);
  }
