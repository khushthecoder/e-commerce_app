...
const ShippingAddressScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    if (user?.uid) {
      const unsubscribe = addressService.subscribeToAddresses(user.uid, setAddresses);
      return () => unsubscribe();
    }
  }, [user]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing) {
        await addressService.updateAddress(user.uid, currentAddress.id, currentAddress);
        Alert.alert('Success', 'Address updated successfully!');
      } else {
        await addressService.addAddress(user.uid, currentAddress);
